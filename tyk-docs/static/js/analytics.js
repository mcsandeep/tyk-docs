		...
		wa = function (a, b, c, d) {
			a &&
				(c
					? ((d = ""),
						b && f.test(b) && (d = ' id="' + b + '"'),
						f.test(a) && M.write("<script" + d + ' src="' + a + '">\x3c/script>'))
		...
		gc = function (a) {
			...
			if (Ec(b) || Fc(b)) {
				...
				void 0 == c ||
					...
					isNaN(c) ||
					(0 < c
						? (Y(b, Gb), Y(b, Jb), Y(b, Ib), Y(b, Fb), Y(b, Hb), Y(b, Kb), Y(b, Lb), a(b))
						: L(
								O,
								"load",
								function () {
	...
	var u = function (a, b, c) {
		...
		y = function (a, b, c, d, e) {
			if (!ea(Yd.get(b)) && !$d.get(b)) {
				...
				c &&
					((c && 0 <= c.indexOf("/")) ||
						(c = (Ba || Ud() ? "https:" : "http:") + "//www.google-analytics.com/plugins/ua/" + c),
					(d = ae(c)),
					(a = d.protocol),
