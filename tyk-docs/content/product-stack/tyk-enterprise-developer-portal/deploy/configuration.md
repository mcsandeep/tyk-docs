...
This section explains how to configure the audit log in the portal. When the audit log is enabled, each admins' action will leave a trace in the _portal.log_ file located at in the directory specified by the `PORTAL_AUDIT_LOG_ENABLE` setting.
...
**Config file:** TLSConfig.Enable <br/>
**Type:** `boolean` <br/>
**Description**: Enables or disables connection over https. When TLS is enabled, the portal will expect a TLS certificate to be provided via _PORTAL_TLS_CERTIFICATES_.
...
**Type:** `[string]` <br/>
**Description**: A list of origin domains to allow access from. Wildcards are also supported, e.g. [`*.foo.com`] will allow access from any domain that ends with _.foo.com_.
