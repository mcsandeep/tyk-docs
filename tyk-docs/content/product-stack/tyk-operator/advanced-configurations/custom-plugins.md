...
In Tyk Classic APIs, the _custom_middleware_ section of the Tyk Classic API Definition is where you configure plugins that will run at different points during the lifecycle of an API request.
...
In this example we will create a JavaScript plugin that will inject a request header _Hello_ with a value of _World_. This will be configured as a pre request [hook]({{< ref "" >}}).
...
The example API Definition resource listed below listens on path _/httpbin_ and forwards requests to upstream *http://httpbin.org*.
