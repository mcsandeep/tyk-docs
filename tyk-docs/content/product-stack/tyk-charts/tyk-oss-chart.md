...
- _Tyk Gateway_: An open source Enterprise API Gateway, supporting REST, GraphQL, TCP and gRPC protocols.
- _Tyk Pump_: An analytics purger that moves the data generated by your Tyk gateways to any back-end storage.
...
_Important Note regarding MongoDB:_ This helm chart enables the PodDisruptionBudget for MongoDB with an arbiter replica-count of 1. If you intend to perform system maintenance on the node where the MongoDB pod is running and this maintenance requires for the node to be drained, this action will be prevented due the replica count being 1. Increase the replica count in the helm chart deployment to a minimum of 2 to remedy this issue.
